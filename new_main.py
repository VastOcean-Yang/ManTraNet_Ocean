...
#     return rgb
def read_rgb_image(image_file):
    rgb = Image.open(image_file).convert('RGB')
    return np.array(rgb)
...
def one_dir_test_and_save_mask(dirs_file_path):
    path_list = os.listdir(dirs_file_path)
    new_dir_path = os.path.join(os.path.dirname(dirs_file_path), os.path.basename(dirs_file_path) + "_mask")
    os.makedirs(new_dir_path, exist_ok=True)
    for path_1 in path_list:
        sub_path = os.path.join(dirs_file_path, path_1)
        rgb, mask, ptime = decode_an_image_file(sub_path, manTraNet)
        print("img saved: "+str(path_1))
        pyplot.imsave(os.path.join(new_dir_path,path_1), mask, cmap='gray')
...
if __name__ == '__main__':
    ...
    one_dir_test_and_save_mask('data/forged')